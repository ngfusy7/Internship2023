...
const modulejsTask2 = function (done) {
  const knownOptions = {
	 string: 'm'
  }
  ...
  _.split(names, ' ').map(name => {
    ...
    const paths = {
      src: [`${root}/gulpfile.js/templates/_module2.js`],
      dest: path.join(config.getPathSrc('scripts'), 'modules')
    }

    const report = (file, cb) => {
      gutil.log(gutil.colors.cyan(`Created2: ${file.path}`))
      cb(null, file)
    }
    const rename = (file, cb) => {
      if (file.basename === '_module2.js') {
...
gulp.task('module2', modulejsTask2)
module.exports = modulejsTask2
